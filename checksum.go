...
func gsoNoneChecksum(buff []byte, start, off uint16) {
	csum := start + off
	initial := binary.BigEndian.Uint16(buff[csum:])
	buff[csum], buff[csum+1] = 0, 0
	binary.BigEndian.PutUint16(buff[csum:], ^checksum(buff[start:], uint64(initial)))
}
...
func checksumValid(pkt []byte, iphLen, proto uint8, isV6 bool) bool {
	...
	cSum := pseudoHeaderChecksumNoFold(proto,
		pkt[srcAddrAt:srcAddrAt+addrSize],
		pkt[srcAddrAt+addrSize:srcAddrAt+addrSize*2], lenForPseudo)
